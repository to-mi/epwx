function [fa, si, converged] = vbep_spca_probit(y, pr, op, fa_or_si_init)
% -- Likelihood:
...
for iter = 1:op.max_iter
    ...
    % update ystar
    ystar_mean = fa.x.Mean' * fa.w.Mean;
    ystar_mean = ystar_mean + y .* exp(-0.5 * ystar_mean.^2) ./ normcdf(y .* ystar_mean) / sqrt(2 * pi);
    
    % update sites of w
    for j = 1:m
        si.lik.w.Tau(:, :, j) = (sum(fa.x.Cov, 3) + fa.x.Mean * fa.x.Mean');
        si.lik.w.Mu(:, j) = fa.x.Mean * ystar_mean(:, j);
    end
    
    % update full approx and ystar
    fa = compute_full_approximation(si, pr);

    % update sites of x
    for i = 1:n
        si.lik.x.Tau(:, :, i) = (sum(fa.w.Cov, 3) + fa.w.Mean * fa.w.Mean');
        si.lik.x.Mu(:, i) = fa.w.Mean * ystar_mean(i, :)';
    end
