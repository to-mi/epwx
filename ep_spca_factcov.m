function [fa, si, converged] = ep_spca_factcov(y, y_lik_type, pr, op, fa_or_si_init)
% -- Likelihood:
%    p(y_ij|x_i,w_j,sigma2_j) = N(y_ij|w_j'x_i, sigma2_j)
% OR p(y_ij|x_i,w_j) = Probit(y_ij w_j'x_i)
% -- Prior:
% p(x_i) = N(x_i|0, I)
...
switch fa_or_si_init.type
    ...
    case 'fa'
        ...
        si.lik.x.Tau = repmat(1/m*bsxfun(@minus, fa_or_si_init.x.Tau, eye(K)), [1 1 1 m]);
        si.lik.x.Mu = repmat(1/m*fa_or_si_init.x.Mu, [1 1 m]);
        si.lik.w.Tau = repmat(1/n*bsxfun(@minus, fa_or_si_init.w.Tau, 1/pr.tau2 * eye(K)), [1 1 1 n]);
        si.lik.w.Mu = repmat(1/n*fa_or_si_init.w.Mu, [1 1 n]);
    otherwise
...
for iter = 1:op.max_iter
    ep_wx_parallelep_factcov(n, m, K, fa, si.lik, zs, y, y_lik_type, pr, op);
...
% K x K x n
fa.x.Tau = repmat(eye(pr.K, pr.K), [1 1 pr.n]) + sum(si.lik.x.Tau, 4);
% K x n
fa.x.Mu = sum(si.lik.x.Mu, 3);

% K x K x m and K x m
fa.w.Tau = zeros(pr.K, pr.K, pr.m);
fa.w.Tau_chol = zeros(pr.K, pr.K, pr.m);
fa.w.Mu = si.prior.w.mu + sum(si.lik.w.Mu, 3);
fa.w.Mean = zeros(pr.K, pr.m);

for i = 1:pr.m
    fa.w.Tau(:, :, i) = diag(si.prior.w.tau(:, i)) + sum(si.lik.w.Tau(:, :, i, :), 4);
    fa.w.Tau_chol(:, :, i) = chol(fa.w.Tau(:, :, i), 'lower');
    fa.w.Mean(:, i) = fa.w.Tau_chol(:, :, i)' \ (fa.w.Tau_chol(:, :, i) \ fa.w.Mu(:, i));
end
